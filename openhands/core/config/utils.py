...
def load_from_toml(cfg: AppConfig, toml_file: str = 'config.toml') -> None:
    ...
    if 'model_routing' in toml_config:
        try:
            model_routing_config = ModelRoutingConfig.from_toml_section(
                toml_config['model_routing']
            )
            cfg.model_routing_config = model_routing_config
        except (TypeError, KeyError, ValidationError) as e:
            logger.openhands_logger.warning(
                f'Cannot parse [model_routing] config from toml, values have not been applied.\nError: {e}'
            )
        except ValueError:
            # Re-raise ValueError from ModelRoutingConfig.from_toml_section
            raise ValueError('Error in [model_routing] section in config.toml')
